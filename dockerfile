#First stage of the multi-stage build:
FROM gradle:8.11.1-jdk21 as builder
WORKDIR /usr/src/ssh_rss
COPY . .
RUN gradle installDist --no-daemon
# This produces a distribution in /usr/src/ssh_rss/build/install/PRP-Recipe-Suggestion-Solution
# which contains bin/ and lib/ directories.

# Second stage: 
FROM openjdk:21-jdk-slim
WORKDIR /app
COPY --from=builder /usr/src/ssh_rss/build/install/PRP-Recipe-Suggestion-Solution/ ./

# Set a default class to be CreateDatabse ratehr than App.java
ENV MAIN_CLASS=org.example.CreateDatabase

# runs the current class
ENTRYPOINT ["sh", "-c", "java -cp 'lib/*' $MAIN_CLASS"]
